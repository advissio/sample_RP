CREATE OR REPLACE PACKAGE VENCD.PA_CD_TARGET AS
...
PROCEDURE PR_ASSOCIA_SALE_VIRTUALI(p_id_cliente CD_PIANIFICAZIONE.ID_CLIENTE%TYPE, p_id_target CD_PRODOTTO_VENDITA.ID_TARGET%TYPE, p_data_inizio CD_PROIEZIONE.DATA_PROIEZIONE%TYPE, p_data_fine CD_PROIEZIONE.DATA_PROIEZIONE%TYPE, p_soglia id_list_type, p_chiamante varchar2);
...
FUNCTION FU_SOGLIA_GIORNO_PRODOTTO(p_id_prodotto_acquistato cd_prodotto_acquistato.id_prodotto_acquistato%TYPE,  p_giorno CD_PROIEZIONE.DATA_PROIEZIONE%TYPE) RETURN NUMBER;-- C_SALE_SETTIMANA;


--PROCEDURE PR_ABBASSA_SOGLIA_PRODOTTO(P_ID_PRODOTTO_ACQUISTATO CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE, P_GIORNO CD_COMUNICATO.DATA_EROGAZIONE_PREV%TYPE );

PROCEDURE PR_ABBASSA_SOGLIA_PROD_GIORNO(P_ID_PRODOTTO_ACQUISTATO CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE, P_GIORNO CD_COMUNICATO.DATA_EROGAZIONE_PREV%TYPE,P_SOGLIA_GIORNO NUMBER );

PROCEDURE PR_ABBASSA_SOGLIA_PRODOTTO(P_ID_PRODOTTO_ACQUISTATO CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE,p_soglia id_list_type);
...
PROCEDURE ASSOCIA_SALE_VIRTUALI(P_DATE DATE default sysdate);

PROCEDURE ASSOCIA_SALE_VIRTUALI_OLD(P_DATE DATE default sysdate);
...
FUNCTION FU_DETT_SALE_GIORNO_IDONEE(
                                    ...
                                    RETURN C_SALA;      
function fu_get_posizione_giorno(p_data_inizio date,p_data_fine date, p_giorno number) return number;                                                                      
