CREATE OR REPLACE PACKAGE BODY VENCD.PA_CD_TARGET AS
/***************************************************************************************
   NAME:      PA_CD_TARGET
...
(SELECT id_sala as num_sala,
        ...
        --
         ...
         PA_CD_ESTRAZIONE_PROD_VENDITA.FU_AFFOLLAMENTO_SALA_STATO_NEW(p_data_inizio,p_data_fine,id_sala) as affollamento
         from 
         (
         select 
...
--
--  MODIFICHE:   Mauro Viel Altran Italia, Ottobre 2010 il richiamo della pr_imposta_posizione avviene solo sui comuncati 
--                                         per cui avviene l'associazione della sala relale e non per tutti.  
--               Mauro Viel Altran Italia, Novrembre 2010 spostato in testa alla procedura il richiamo alla procedura PR_ANNULLA_SALE_NON_ASSEGNATE.
--
-------------------------------------------------------------------------------------------------
PROCEDURE PR_ASSOCIA_SALE_VIRTUALI(p_id_cliente CD_PIANIFICAZIONE.ID_CLIENTE%TYPE, p_id_target CD_PRODOTTO_VENDITA.ID_TARGET%TYPE, p_data_inizio CD_PROIEZIONE.DATA_PROIEZIONE%TYPE, p_data_fine CD_PROIEZIONE.DATA_PROIEZIONE%TYPE, p_soglia id_list_type,p_chiamante varchar2) IS--p_soglia cd_schermo_virtuale_prodotto.SOGLIA%type) IS
--v_sale_associabili C_SALE_ASSOCIABILI;
...
BEGIN
    ...
    LOOP
          if (p_chiamante ='web') then
            PR_ABBASSA_SOGLIA_PRODOTTO(PR_TARGET.ID_PRODOTTO_ACQUISTATO, v_soglia);
          end if;
          PR_ANNULLA_SALE_NON_ASS_NEW(PR_TARGET.ID_PRODOTTO_ACQUISTATO, PR_TARGET.STATO_DI_VENDITA, PR_TARGET.ID_PRODOTTO_VENDITA, PR_TARGET.ID_CIRCUITO, PR_TARGET.ID_TARGET);
    ...
    FOR PR_TARGET IN(
        ...
        LOOP
            ...
            i := i + 1;
            dbms_output.PUT_LINE('gg='||gg.giorno);
        END LOOP;    
        --    
        v_soglia := p_soglia;
        ...
        END IF;
        --
        -- dbms_output.PUT_LINE('PR_TARGET.ID_PRODOTTO_ACQUISTATO='||PR_TARGET.ID_PRODOTTO_ACQUISTATO);
    ...
    LOOP
        ...
        --    EXIT WHEN v_prodotti_target%NOTFOUND;
           ...
           -- dbms_output.PUT_LINE('v_esito='||v_esito);
...
--    CLOSE v_prodotti_target;
   ...
   /* --Ciclo prodotti
    ...
    LOOP  
         ---PR_ANNULLA_SALE_NON_ASSEGNATE(PR_TARGET.ID_PRODOTTO_ACQUISTATO, PR_TARGET.STATO_DI_VENDITA, PR_TARGET.ID_PRODOTTO_VENDITA, PR_TARGET.ID_CIRCUITO, PR_TARGET.ID_TARGET);
        --PA_CD_PRODOTTO_ACQUISTATO.PR_IMPOSTA_POSIZIONE(PR_TARGET.ID_PRODOTTO_ACQUISTATO, null);
    END LOOP;*/
...
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE: MAuro Viel Altrna Italia Sostituita la condizione
--                                  IF v_sale_giorno_rec.a_num_schermi < p_soglia(v_list_giorni_da_associare(i)) THEN con
--                                  IF v_sale_giorno_rec.a_num_schermi < p_soglia(fu_get_posizione_giorno(v_data_inizio,v_data_fine,v_list_giorni_da_associare(i))) THEN
--                               Per consentire la gestioen dei periodi speciali (non settimane Sipra)     
--
-------------------------------------------------------------------------------------------------
                        ...
                        --
                            ...
                            (
                               ...
                               --dbms_output.PUT_LINE('COM.ID_FASCIA: '||COM.ID_FASCIA);
                                ...
                                --Verifico se la posizione individuata e di rigore questa condizione 
                                --puo verificarsi solo se:
                                --Viene associato il prodotto a Target. 
                                --Viene disassociata una sala con una posizione X.
                                --Viene prenotato un prodotto con posizione di rigore X.
                                --Il prodotto a target viene riassociato : in questo caso sulla posizione storica risiede la poosizione X.
                                --Nella realta prima tutti i prodotti vengono prenotati e poi avviene l'associazione e la disassociazione del target.
   
                                select count(1)
                                into v_esiste_pos_rigore
                                from cd_comunicato
                                where id_break = v_id_break
                                and  posizione_di_rigore  = v_posizione; 
        
                               

                                if v_posizione >0 and  v_esiste_pos_rigore = 0 then 
                                    --dbms_output.PUT_LINE('imposto la  posizione CON PREFERENZA posizione: '|| v_posizione);
                                    PA_CD_PRODOTTO_ACQUISTATO.PR_IMPOSTA_POSIZIONE(p_id_prodotto_acquistato,COM.ID_COMUNICATO,v_posizione);
                                else
                                     --dbms_output.PUT_LINE('imposto la  posizione senza preferenza ');
                                    PA_CD_PRODOTTO_ACQUISTATO.PR_IMPOSTA_POSIZIONE(p_id_prodotto_acquistato,COM.ID_COMUNICATO);
                                end if;    
...
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE:  Mauro  Viel  Modificata la condizione di update dello schermo virtale prodotto per la gestione dei periodi speciali (settimane non sipra)
--             inserita la condizioen sulla data_inizio. 
--
-------------------------------------------------------------------------------------------------
    ...
    ELSE
        IF p_soglia IS NOT NULL THEN
            FOR i IN p_soglia.first..p_soglia.last LOOP
                ...
                --AND i = to_char(v_data_inizio,'D');
                AND giorno = v_data_inizio;
                v_data_inizio := v_data_inizio + 1;
             END LOOP;
        END IF;
    END IF;
    --
...
BEGIN
    ...
    OPEN v_sale_idonee FOR
       select /*+ RULE*/ distinct (data_proiezione) as data_proiezione, max(NUM_SALE) AS NUM_SALE
       from
       (
...
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE:  Antonio Colucci Teoresio srl, 01/03/2011
--                  Ottimizzazione della query: lasciato inalterato il core, 
--                  tolta la chiamata alla funzione FU_AFFOLLAMENTO_SALA_STATO_NEW e sostituita
--                  con una WITH
-------------------------------------------------------------------------------------------------
...
BEGIN
    OPEN v_sale_idonee FOR
       ...
       (
           ...
           PA_CD_ESTRAZIONE_PROD_VENDITA.FU_AFFOLLAMENTO_SALA_STATO_NEW(DATA_PROIEZIONE, DATA_PROIEZIONE, ID_SALA) AS DISPONIBILITA
           from 
           (
               select 
                 sa.ID_SALA, 
   ...
   --     WHERE DISPONIBILITA > 0
        ...
        */
        select /*+ RULE*/ distinct 
               (data_proiezione),
               max(NUM_SALE) over (partition by data_proiezione) AS NUM_SALE
           from
           (
           ------------------------------------------------------------
           select distinct
           ...
           (    with sala_affollamento as
                ...
                select
                    ...
                    select   distinct
                             ...
                             (sum(distinct cd_tipo_break.durata_secondi) over (partition by cd_break.id_proiezione)
                             - 
...
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE:  Mauro Viel, Altran Italia, Novembre 2010
--              modifica la condizione delle sale a programmazione parziale. #MVNOV01
-------------------------------------------------------------------------------------------------
...
v_id_break_vendita CD_COMUNICATO.ID_BREAK_VENDITA%TYPE;
v_id_break CD_COMUNICATO.ID_BREAK%TYPE;
v_num_comunicati NUMBER := 0;
BEGIN
--dbms_output.PUT_LINE('p_id_prodotto_acquistato: '||p_id_prodotto_acquistato);
...
--dbms_output.PUT_LINE('p_id_target: '||p_id_target);        
    ...
    (
        ...
        (
                ...
                AND 
                (
                    SPE.ID_SPETTACOLO IN --#MVNOV01 AND SPE.ID_SPETTACOLO NOT IN  
                    (
                        SELECT ID_SPETTACOLO
                        FROM CD_SPETT_TARGET
                    )
...
--              modifica la condizione delle sale a programmazione parziale. #MVNOV01
-------------------------------------------------------------------------------------------------
PROCEDURE PR_ANNULLA_SALE_NON_ASS_NEW(p_id_prodotto_acquistato CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE, p_stato_vendita CD_PRODOTTO_ACQUISTATO.STATO_DI_VENDITA%TYPE, p_id_prodotto_vendita CD_PRODOTTO_VENDITA.ID_PRODOTTO_VENDITA%TYPE, p_id_circuito CD_CIRCUITO.ID_CIRCUITO%TYPE, p_id_target CD_TARGET.ID_TARGET%TYPE) IS
--
v_id_sala_virtuale CD_SALA.ID_SALA%TYPE;
v_id_break_vendita CD_COMUNICATO.ID_BREAK_VENDITA%TYPE;
v_id_break CD_COMUNICATO.ID_BREAK%TYPE;
    ...
    LOOP
        ...
        ------------------------------------------------------------------------
            ...
            if V_ROWNUM = 0  then
                ...
                and   co.DATA_EROGAZIONE_PREV = SALA.GIORNO
...
END PR_ANNULLA_SALE_NON_ASS_NEW;
...
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE: Mauro Viel  Altran Italia: inserita la update sulla colonna 
--                                         cd_prodotto_acquistato.flg_ricalcolo_tariffa
--             Mauro Viel, Altran Giugno 2011 : Inserito l'annullamento delle sale virtuali dopo 
--                                              il ricalcolo della tariffa.
-- Mauro Viel,  Altran Novembre 2011: inserito l'aggiornamneto della sintesi prod_acq il modo che se il ricalcolo della 
--                               tariffa avviene dopo il consolidamento sulla tavola cd_sintesi_prod_acq il numero di schermi 
--                               vengono conteggiati con le sole sale reali allocate per il prodotto.
-------------------------------------------------------------------------------------------------
...
BEGIN
    ...
    (
        ...
        (
            ...
            and   com.id_prodotto_acquistato = pacq.id_prodotto_acquistato 
            and   ci.id_cinema = sa.id_cinema
            and   ci.flg_virtuale = 'S'
        ); 
        
               --Aggiorno cd_sintesi prododdo dopo l'annullamento delle sale.
        pa_cd_prodotto_acquistato.pr_aggiorna_sintesi_prodotto(pacq.id_prodotto_acquistato);                                                            
...
-----------------------------------------------------------------------------------------------------
-- Funzione FU_GET_SALA_TARGET_GIORNO
--
-- DESCRIZIONE:  Restituisce una sala random tra quelle idonee e disponibili
--              per l'associazione delle sale nei prodotti a target
--
-- REALIZZATORE: Simone Bottani, Altran, Luglio 2010
--
--  MODIFICHE:MV 24/08/2010 aggiunta la group by x disponibilita perche utilizata nella having
--            Antonio Colucci Teoresio srl, 01/03/2011
--                  Ottimizzazione della query: lasciato inalterato il core, 
--                  tolta la chiamata alla funzione FU_AFFOLLAMENTO_SALA_STATO_NEW e sostituita
--                  con una WITH
--
...
/*
       ...
       (
           ...
           PA_CD_ESTRAZIONE_PROD_VENDITA.FU_AFFOLLAMENTO_SALA_STATO_NEW(p_giorno, p_giorno, ID_SALA) AS DISPONIBILITA
           from
           (
               select 
           ...
           (
               with sala_affollamento as
                ...
                select
                    ...
                    and     cd_comunicato.cod_disattivazione is null
                    and     cd_comunicato.flg_sospeso = 'N'
                    and     cd_comunicato.data_erogazione_prev between p_giorno and p_giorno
                    and     cd_comunicato.id_prodotto_acquistato = cd_prodotto_acquistato.id_prodotto_acquistato
                    and     cd_prodotto_acquistato.stato_di_vendita = 'PRE'
                    and     cd_prodotto_acquistato.id_formato = cd_formato_acquistabile.id_formato
                    and     cd_formato_acquistabile.id_coeff = cd_coeff_cinema.id_coeff
               ...
               select bacino_sale.id_sala,affollamento as disponibilita
               from
               (
                   select 
                     sa.ID_SALA, sa.NOME_SALA
...
--Mauro Viel Altran Italia Maggio 2011 inserito p_id_cliente nella clausola where. 
--In precedenza il parametro non era utilizzato. Per questo motivo il parametro e opzionale. 
...
BEGIN
     ...
     (
         ...
         FROM CD_PRODOTTO_ACQUISTATO PA, CD_SCHERMO_VIRTUALE_PRODOTTO SVP, CD_PRODOTTO_VENDITA PV,CD_PIANIFICAZIONE PIA
...
FUNCTION FU_SOGLIA_GIORNO_PRODOTTO(p_id_prodotto_acquistato cd_prodotto_acquistato.id_prodotto_acquistato%TYPE,  p_giorno CD_PROIEZIONE.DATA_PROIEZIONE%TYPE) RETURN NUMBER IS--RETURN C_SALE_SETTIMANA IS
V_SOGLIA NUMBER;    
BEGIN
     SELECT  SOGLIA INTO V_SOGLIA
     ...
     AND   PA.FLG_SOSPESO = 'N'
     AND   PA.COD_DISATTIVAZIONE IS NULL
     AND   SVP.GIORNO = P_GIORNO
     AND   ROWNUM=1;
RETURN V_SOGLIA;
END FU_SOGLIA_GIORNO_PRODOTTO;


PROCEDURE PR_ABBASSA_SOGLIA_PROD_GIORNO(P_ID_PRODOTTO_ACQUISTATO CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE, P_GIORNO CD_COMUNICATO.DATA_EROGAZIONE_PREV%TYPE,P_SOGLIA_GIORNO NUMBER ) IS
v_sale_giorno_associate pa_cd_target.C_SALE_SETTIMANA;
v_sale_giorno_rec pa_cd_target.R_SALE_SETTIMANA;
v_sale number; 
v_diff_sale number:= 0;
v_id_sala_virtuale CD_SALA.ID_SALA%TYPE;
v_id_break_vendita CD_COMUNICATO.ID_BREAK_VENDITA%TYPE;
v_id_break CD_COMUNICATO.ID_BREAK%TYPE;
v_num_comunicati NUMBER := 0;
...
v_rownum number := 1;
begin
v_sale := p_soglia_giorno;
v_sale_giorno_associate  := FU_SALE_GIORNO(p_id_prodotto_acquistato,p_giorno);
LOOP
FETCH v_sale_giorno_associate INTO v_sale_giorno_rec;
EXIT WHEN v_sale_giorno_associate%NOTFOUND;
    v_diff_sale :=  v_sale_giorno_rec.a_num_schermi - v_sale; 
    if v_diff_sale > 0 then
    
    
    select cir.id_circuito, pv.id_prodotto_vendita, pa.stato_di_vendita  
    into   v_id_circuito, v_id_prodotto_vendita,v_stato_vendita 
    from   cd_prodotto_vendita pv, cd_circuito cir, cd_prodotto_acquistato pa
    where  pa.ID_PRODOTTO_ACQUISTATO = p_id_prodotto_acquistato
    and    pa.ID_PRODOTTO_VENDITA = pv.ID_PRODOTTO_VENDITA
    and    pv.ID_CIRCUITO = cir.ID_CIRCUITO;
    ...
    AND ROWNUM <= 1;*/
        ...
        (
            SELECT * FROM
            (
                SELECT ID_COMUNICATO, ID_FASCIA, COM.ID_SALA
                FROM CD_PROIEZIONE PR, CD_BREAK BR, CD_COMUNICATO COM, CD_SALA SA, CD_CINEMA CI
                WHERE COM.ID_PRODOTTO_ACQUISTATO = p_id_prodotto_acquistato
            ...
            --AND ROWNUM <= 2*  v_diff_sale --devo virtualizzare due comunicati
               order by id_sala
            )
            where  ROWNUM <= 2*  v_diff_sale --devo virtualizzare due comunicati
        ...
        LOOP
            ...
            /*dbms_output.PUT_LINE('id_comunicato = '|| com.id_comunicato);
            dbms_output.PUT_LINE('id_sala = '|| com.id_sala);
             dbms_output.PUT_LINE('v_id_sala_virtuale = '|| v_id_sala_virtuale);
            dbms_output.PUT_LINE('v_id_break_vendita = '|| v_id_break_vendita);
            dbms_output.PUT_LINE('v_id_break = '|| v_id_break);*/
        ...
        END Loop;
        IF v_num_comunicati = 0 THEN
            RAISE_APPLICATION_ERROR(-20025, 'PR_ABBASSA_SOGLIA_PRODOTTO DURANTE L'' ESECUZIONE DELLA PROCEDURA,  '||SQLERRM);
        END IF;     
    
    end if;
END LOOP;
END PR_ABBASSA_SOGLIA_PROD_GIORNO;

PROCEDURE PR_ABBASSA_SOGLIA_PRODOTTO(P_ID_PRODOTTO_ACQUISTATO CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE,p_soglia id_list_type) IS
V_DATA_INIZIO DATE; 
V_DATA_FINE   DATE;
v_giorno date;
v_soglia_giorno number; 
BEGIN
...
FOR I IN 0..v_data_fine - v_data_inizio
LOOP
    v_giorno := v_data_inizio + i;
    if  p_soglia is null then
        v_soglia_giorno :=  FU_SOGLIA_GIORNO_PRODOTTO(p_id_prodotto_acquistato,v_giorno);
    else
        v_soglia_giorno := (p_soglia(i+1));   
    end if;
    if v_giorno >= trunc(sysdate) then
        PR_ABBASSA_SOGLIA_PROD_GIORNO(p_id_prodotto_acquistato,v_giorno,v_soglia_giorno);  
    end if;
END LOOP;
END PR_ABBASSA_SOGLIA_PRODOTTO;


-- Mauro Viel  Altran Italia Maggio 2010. La revisione della procedura lavorera ogni singolo prodotto del cliente.
-- La modifica si rende necessaria per l'introduzione dei prodotti a recupero dove per un target si potranno avere 
-- diverse soglie di associazione.   
--Mauro Viel Altran Italia Settembre 2011 inseriti i prodotti su periodi speciali. La nuova caratteristica si rende necessaria 
--per l'introduzione a magazzino di prodotti su periodi speciali. #MV01
 
PROCEDURE ASSOCIA_SALE_VIRTUALI(P_DATE DATE default sysdate) IS
v_soglia id_list_type := id_list_type();
j number:= 0;
--V_DATA_INIZIO DATE;
--V_DATA_FINE DATE;
v_soglie_count number;
BEGIN
 ...
 INTO V_DATA_INIZIO, V_DATA_FINE
 FROM PERIODI
 WHERE TRUNC(P_DATE) BETWEEN DATA_INIZ AND DATA_FINE; #MV01*/
 
 for c_date in
 (
     SELECT DATA_INIZ as DATA_INIZIO, DATA_FINE
     FROM PERIODI
     WHERE TRUNC(P_DATE) BETWEEN DATA_INIZ AND DATA_FINE
     union 
     select distinct ps.data_inizio,ps.data_fine
     from cd_periodo_speciale ps,
          cd_prodotto_acquistato pa,
          cd_prodotto_vendita pv,
          cd_importi_richiesti_piano irp
     WHERE TRUNC(P_DATE) BETWEEN ps.DATA_INIZIO AND ps.DATA_FINE
     and   pa.id_prodotto_vendita = pv.id_prodotto_vendita
     and   pa.flg_annullato = 'N'
     and   pa.flg_sospeso = 'N'
     and   pa.cod_disattivazione is null
     and   pa.STATO_DI_VENDITA = 'PRE'
     and   pa.ID_IMPORTI_RICHIESTI_PIANO = irp.ID_IMPORTI_RICHIESTI_PIANO
     and   irp.ID_PERIODO_SPECIALE = ps.ID_PERIODO_SPECIALE
     and   pv.ID_TARGET is not null
 )
 loop
     for target in (SELECT distinct pv.id_target,pia.id_cliente 
     FROM CD_PRODOTTO_VENDITA PV, CD_PRODOTTO_ACQUISTATO PA, CD_PIANIFICAZIONE PIA
     WHERE PA.DATA_INIZIO = c_date.data_inizio--v_data_inizio
     AND PA.DATA_FINE = c_date.data_fine--v_data_fine
     AND PA.FLG_ANNULLATO = 'N'
     AND PA.FLG_SOSPESO = 'N'
     AND PA.COD_DISATTIVAZIONE IS NULL
     AND PA.STATO_DI_VENDITA = 'PRE'
     ...
     AND PIA.FLG_ANNULLATO = 'N'
     AND PIA.FLG_SOSPESO = 'N'
     AND PIA.ID_CLIENTE = NVL(null,PIA.ID_CLIENTE)
     AND PV.ID_PRODOTTO_VENDITA = PA.ID_PRODOTTO_VENDITA
     AND PV.ID_TARGET IS NOT NULL
     --AND PV.ID_TARGET = NVL(null, PV.ID_TARGET)
     order by pv.id_target)
     loop
         v_soglia := id_list_type();
         j := 1; 
         --dbms_output.put_line('target='||target.id_target);
         --dbms_output.put_line('cliente='||target.id_cliente);
         for soglia in (SELECT DISTINCT SOGLIA, GIORNO
                 FROM CD_PRODOTTO_ACQUISTATO PA, 
                      CD_SCHERMO_VIRTUALE_PRODOTTO SVP, 
                      CD_PRODOTTO_VENDITA PV,
                      CD_PIANIFICAZIONE PIA
                 WHERE PA.ID_PRODOTTO_ACQUISTATO = SVP.ID_PRODOTTO_ACQUISTATO
                 AND PA.FLG_ANNULLATO = 'N'
                 ...
                 AND PA.COD_DISATTIVAZIONE IS NULL
                 AND PV.ID_PRODOTTO_VENDITA = PA.ID_PRODOTTO_VENDITA
                 AND PV.ID_TARGET = target.id_target
                 AND GIORNO BETWEEN c_date.data_inizio and c_date.data_fine--V_DATA_INIZIO AND V_DATA_FINE
                 AND PIA.ID_PIANO = PA.ID_PIANO
                 AND PIA.ID_VER_PIANO = PA.ID_VER_PIANO
                 AND PIA.ID_CLIENTE = NVL(target.id_cliente,PIA.ID_CLIENTE)
                 GROUP BY GIORNO, SOGLIA
                 ORDER BY GIORNO)
         loop      
             v_soglia.extend;
             v_soglia(j) := soglia.soglia;
             --dbms_output.put_line('j='||j);
             --dbms_output.put_line('v_soglia:'||v_soglia(j));
             j := j + 1;
         end loop;
         ---gestione dei prodotti a tagli temporale diverso dalla settimana.
         if(v_soglia.count < 7) then
            v_soglie_count := v_soglia.count;
            for i in v_soglie_count..7
            loop
              v_soglia.extend;
              v_soglia(i) :=  v_soglia(v_soglie_count); 
            end loop;
         end if;
         if(v_soglia.count != 0) then
             --dbms_output.put_line('Target:'||target.id_target || ', Cliente:'|| target.id_cliente || ', data inizio:'|| c_date.data_inizio ||', data fine:' || c_date.data_fine  );
             --PR_ASSOCIA_SALE_VIRTUALI(target.id_cliente, target.id_target, V_DATA_INIZIO, V_DATA_FINE, v_soglia,'DB');
             PR_ASSOCIA_SALE_VIRTUALI(target.id_cliente, target.id_target,  c_date.data_inizio,  c_date.data_fine, v_soglia,'DB');
            
         end if;
         commit;
     end loop;
end loop;     
END ASSOCIA_SALE_VIRTUALI;




PROCEDURE ASSOCIA_SALE_VIRTUALI_OLD(P_DATE DATE default sysdate) IS
v_soglia id_list_type := id_list_type();
j number:= 0;
V_DATA_INIZIO DATE;
V_DATA_FINE DATE;
...
BEGIN
 ...
 for target in (SELECT distinct pv.id_target 
 FROM CD_PRODOTTO_VENDITA PV, CD_PRODOTTO_ACQUISTATO PA, CD_PIANIFICAZIONE PIA
 WHERE PA.DATA_INIZIO = v_data_inizio
 ...
 loop
     ...
     --dbms_output.put_line('target='||target.id_target);
     for soglia in (SELECT DISTINCT SOGLIA, GIORNO
             FROM CD_PRODOTTO_ACQUISTATO PA, CD_SCHERMO_VIRTUALE_PRODOTTO SVP, CD_PRODOTTO_VENDITA PV
             WHERE PA.ID_PRODOTTO_ACQUISTATO = SVP.ID_PRODOTTO_ACQUISTATO
             AND PA.FLG_ANNULLATO = 'N'
             AND PA.FLG_SOSPESO = 'N'
             AND PA.COD_DISATTIVAZIONE IS NULL
             AND PV.ID_PRODOTTO_VENDITA = PA.ID_PRODOTTO_VENDITA
             AND PV.ID_TARGET = target.id_target
             AND GIORNO BETWEEN V_DATA_INIZIO AND V_DATA_FINE
             GROUP BY GIORNO, SOGLIA
             ORDER BY GIORNO)
     loop      
         v_soglia.extend;
         v_soglia(j) := soglia.soglia;
         --dbms_output.put_line('j='||j);
         --dbms_output.put_line('v_soglia(j)='||v_soglia(j));
         j := j + 1;
     end loop;
     ---gestione dei prodotti a tagli temporale diverso dalla settimana.
     if(v_soglia.count < 7) then
     ...
     if(v_soglia.count != 0) then
         --dbms_output.put_line('chiamo la procedura');
        PR_ASSOCIA_SALE_VIRTUALI(NULL, target.id_target, V_DATA_INIZIO, V_DATA_FINE, v_soglia,'DB');
     end if;
     commit;
 end loop;
END ASSOCIA_SALE_VIRTUALI_OLD;

--Restituisce 0 se la soglia e univoca un numero diverso da zero altrimenti.
FUNCTION FU_VERIFICA_UNIVOCITA_SOGLIA(p_id_target cd_prodotto_vendita.id_target%type,
...
V_NUMERO_SOGLIE NUMBER;
BEGIN 

         --dbms_output.PUT_LINE('1');
         select count(1)-(p_data_fine-p_data_inizio+1) as diff
         into V_NUMERO_SOGLIE
         from
         (
             SELECT DISTINCT SOGLIA, GIORNO
             FROM CD_PRODOTTO_ACQUISTATO PA, CD_SCHERMO_VIRTUALE_PRODOTTO SVP, CD_PRODOTTO_VENDITA PV, cd_pianificazione pia
       ...
       V_NUMERO_SOGLIE := ABS(V_NUMERO_SOGLIE);
       IF V_NUMERO_SOGLIE = 7 THEN
          V_NUMERO_SOGLIE:= 0;  
       END IF;
RETURN V_NUMERO_SOGLIE;        
         
END FU_VERIFICA_UNIVOCITA_SOGLIA;
...
--
-- REALIZZATORE:
--              Tommaso D'Anna, Teoresi srl, 29 Luglio 2011
-------------------------------------------------------------------------------------------------
...
BEGIN
    OPEN C_RETURN FOR
        ...
        FROM    
            ...
            CD_CINEMA,        
            CD_COMUNE
        WHERE LISTA_SALE.ID_SALA                = CD_SALA.ID_SALA
...
-- DESCRIZIONE:  
--              Funzione che permette di recuperare la lista (dalla data inserita in input)
--              delle sale disponibili per il target selezionato
--
-- INPUT:
--              p_data_proiezione           data di ricerca
--              p_id_target                 l'id del target
                        ...
                        WITH SALA_AFFOLLAMENTO AS
                             (
                                 SELECT
                                     DISTINCT ID_SALA,  
                                     MIN(DISPONIBILITA) 
                                        OVER (PARTITION BY ID_SALA) AS AFFOLLAMENTO
                                     FROM
                                     (
                                         SELECT 
                                            DISTINCT CD_COMUNICATO.ID_SALA,
                                                (
                                                    SUM(DISTINCT CD_TIPO_BREAK.DURATA_SECONDI) OVER (PARTITION BY CD_BREAK.ID_PROIEZIONE)
                                                    - 
                                                    SUM (CD_COEFF_CINEMA.DURATA) OVER (PARTITION BY CD_BREAK.ID_PROIEZIONE)
                                                ) AS DISPONIBILITA
                                         FROM    CD_SALA,CD_CINEMA,
                                                 CD_COMUNICATO,
                                                 CD_PRODOTTO_ACQUISTATO,
                            ...
                            (
                                SELECT 
                                    ...
                                    PRO.DATA_PROIEZIONE
                                FROM 
                                    CD_CONDIZIONE_CONTRATTO COND, 
                                    CD_CONTRATTO CON, 
                                    CD_CINEMA_CONTRATTO CC, 
                                    CD_CINEMA CIN,
                                    CD_SALA SA,CD_SCHERMO SCH, 
                                    CD_PROIEZIONE PRO, 
                                ...
                                AND SPT.ID_SPETTACOLO           = SPE.ID_SPETTACOLO
                                AND TAR.ID_TARGET               = SPT.ID_TARGET
                                AND PRO.DATA_PROIEZIONE         = p_data_proiezione
                                AND TAR.ID_TARGET               = NVL(p_id_target,TAR.ID_TARGET)
                                AND CIN.ID_CINEMA               = SA.ID_CINEMA
                                AND CC.ID_CINEMA                = CIN.ID_CINEMA
                                AND CON.ID_CONTRATTO            = CC.ID_CONTRATTO
...
-- DESCRIZIONE:  
--              Funzione che permette di recuperare la lista (dalla data inserita in input)
--              delle sale idonee per il target selezionato
--
-- INPUT:
--              p_data_proiezione           data di ricerca
--              p_id_target                 l'id del target
...
---------------------------------------------------------------------------------------------------
-- FUNCTION fu_get_posizione_giorno
--
-- DESCRIZIONE:  
--              Consente il reperimento dell'indice corretto della soglia anche per i periodi che non sono settimane Sipra.
--
--  Realizzatore Mauro Viel Altran Italia
--              
function fu_get_posizione_giorno(p_data_inizio date,p_data_fine date, p_giorno number) return number is
v_posizione number := 1;
v_data date := p_data_inizio;
begin
    while v_data != p_data_fine
    loop
        if to_char(v_data,'D') = p_giorno then
            return v_posizione;
        end if; 
