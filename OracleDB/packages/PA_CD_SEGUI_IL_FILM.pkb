CREATE OR REPLACE PACKAGE BODY VENCD.PA_CD_SEGUI_IL_FILM AS
...
--              Mauro Viel Altran Italia, Ottobre 2011
--                  -Inserita la possibilita di selezionare le sale a disponibilita parziale MV#01
-------------------------------------------------------------------------------------------------
        ...
        if(v_count_cliente = 0) then
            ...
            --sale DISPONIBILI NON ASSOCIATE  
             select count(id_sala) into v_count_sale_disp
                ...
                --and     spett_sala_giorno.num_spett_giorno = 1 --MV#01
                and     spett_sala_giorno.num_spett_giorno = decode(nvl(cd_prodotto_acquistato.flg_esclusivo,'S'),'S', 1,'N',spett_sala_giorno.num_spett_giorno) --MV#01
...
--
--  MODIFICHE: Mauro Viel Altran Italia inserita tracciatura della modalita si scelta delle sale esclusiva parziale MV#02
-------------------------------------------------------------------------------------------------
...
v_soglia                number;  
v_flg_esclusivo         cd_prodotto_acquistato.flg_esclusivo%type;--MV#02               
  begin
    P_ESITO := 1;
        --MV#02
...
--    
    ...
    if(p_soglia is not null)then
        ...
        set     soglia = p_soglia,
                flg_esclusivo = v_flg_esclusivo--MV#02
        where   id_prodotto_acquistato = p_id_prodotto_acquistato
        and     giorno = p_giorno;
    else
...
--                           
 ...
 v_exit                   number := 0;
 v_flg_esclusivo          cd_prodotto_acquistato.flg_esclusivo%type;--MV#01
 begin
        ...
        */
        ...
        if(v_sala_virtuale <> 0) then
            ...
            (
                ...
                where   spett_sala_giorno.id_schermo = cd_schermo.id_schermo
                --and     spett_sala_giorno.num_spett_giorno = 1 --MV#01
                and     spett_sala_giorno.num_spett_giorno = decode(nvl(v_flg_esclusivo,'S'),'S', 1,'N',spett_sala_giorno.num_spett_giorno)
...
--    
    ...
    for k IN 0..v_num_giorni LOOP
        ...
        (id_prodotto_acquistato,id_sala,giorno,soglia,flg_virtuale,flg_esclusivo)--MV#02
        values
        (p_id_prodotto_acquistato,elenco_sale.id_sala,p_data_inizio+k,nvl(p_soglia,90),'S',v_flg_esclusivo);--MV#02
        END LOOP;
        if(p_data_inizio = p_data_fine)then --sto trattando un solo giorno
           exit;
        end if;
...
BEGIN
        ...
        pa_cd_prodotto_acquistato.pr_aggiorna_sintesi_prodotto(p_id_prodotto_acquistato);                               
...
--              Tommaso D'Anna, Teoresi srl, 28 Ottobre 2011
--                  -   Aggiunto recupero dell'informazione relativa al FLG_ESCLUSIVO del prodotto
-------------------------------------------------------------------------------------------------
FUNCTION FU_RICERCA_PROD_SEGUI_IL_FILM  (
    ...
    FOR
        ...
        (
            SELECT DISTINCT
                ...
                CD_PRODOTTO_ACQUISTATO.FLG_ESCLUSIVO,
...
--              Tommaso D'Anna, Teoresi srl, 23 Giugno 2011
--                  - Inserito il controllo sul contratto valido
--              Mauro Viel Altran Italia inserito p_esclusivo per consentire l'estrazioen delle sale a programmazioen esclusiva o non eslusiva
-------------------------------------------------------------------------------------------------
    ...
    OPEN C_RETURN FOR
        SELECT DISTINCT
            ...
            FROM (
                ...
                WHERE   SPETT_SALA_GIORNO.ID_SCHERMO = CD_SCHERMO.ID_SCHERMO
                --AND     SPETT_SALA_GIORNO.NUM_SPETT_GIORNO = 1 --MV#01
                AND     spett_sala_giorno.num_spett_giorno = decode(p_esclusivo,'S', 1,'N',spett_sala_giorno.num_spett_giorno)
                AND     SPETT_SALA_GIORNO.ID_SPETTACOLO = p_id_spettacolo
                    --UNION
                UNION
                    --UNION
...
--              Tommaso D'Anna, Teoresi srl, 23 Giugno 2011
--                  - Inserito il controllo sul contratto valido
--              Mauro Viel Altran Italia inserito p_esclusivo per consentire l'estrazione delle sale a programmazione esclusiva o non eslusiva --#MV01
--              Tommaso D'Anna, Teoresi srl, 2 Novembre 2011
--                  - Inserita HINT RULE per velocizzare la query
--              Mauro Viel Altran Italia Dicembre 2011 Inserito calcolo del secondaggio break --#MV02
...
IS
C_RETURN C_NUM_SALE_DATA;
v_durata_break number:= 540.5;  --#MV02
BEGIN

    select sum(durata_secondi)+0.5  --#MV02
    into v_durata_break
    from  cd_tipo_break
         ...
         SELECT /*+ RULE*/ DISTINCT
            ...
            FROM (    
                ...
                (
                    ...
                    --540 TOT_GIORNO
                    v_durata_break TOT_GIORNO  --#MV02
                     from(
                       SELECT DISTINCT
                                 ID_SALA,
                                 CD_COMUNICATO.DATA_EROGAZIONE_PREV,
                                 --(540 - SUM(CD_COEFF_CINEMA.DURATA) OVER (PARTITION BY CD_BREAK.ID_PROIEZIONE)) AS DISPONIBILITA
                                 (v_durata_break - SUM(CD_COEFF_CINEMA.DURATA) OVER (PARTITION BY CD_BREAK.ID_PROIEZIONE)) AS DISPONIBILITA 
                        FROM    CD_COMUNICATO,
                                CD_PRODOTTO_ACQUISTATO,
                                CD_FORMATO_ACQUISTABILE,
                                CD_COEFF_CINEMA,
                        ...
                        select distinct
                            ...
                            v_durata_break as disponibilita  --#MV02
                    	from
                    		cd_sala,
                            cd_cinema,
                            cd_schermo,
...
--              Mauro Viel Altran Italia inserito p_esclusivo per consentire l'estrazione delle sale a programmazione esclusiva o non eslusiva
-------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
-- PROCEDURE PR_AGGIORNA_MOD_SCELTA_SALE
--
-- DESCRIZIONE: Consente la variazione della modalitadi scelta delle sale a proiezione esclusiva o parziale del film scelto.  
--              i valori possibili sono : 'S' oppure 'N'; 
--              'S' indica la modalita esclusiva. Operativamente la procedura aggiorna il campo FLG_ESCLUSIVO
--              della tavola cd_prodotto_acquistato per il prodotto indicato.
--
-- INPUT:
--  
--
-- REALIZZATORE:  Mauro Viel Altran Italia, Ottobre 2011 
-------------------------------------------------------------------------------------------------
PROCEDURE PR_AGGIORNA_MOD_SCELTA_SALE(
                                        p_id_prodotto_acquistato    CD_PRODOTTO_ACQUISTATO.ID_PRODOTTO_ACQUISTATO%TYPE,
                                        p_esclusivo                 CD_PRODOTTO_ACQUISTATO.FLG_ESCLUSIVO%TYPE                                          
                                      ) IS
BEGIN 
    UPDATE CD_PRODOTTO_ACQUISTATO
    SET    FLG_ESCLUSIVO            = p_esclusivo
    WHERE  ID_PRODOTTO_ACQUISTATO   = p_id_prodotto_acquistato;
END PR_AGGIORNA_MOD_SCELTA_SALE;                                          
--
END PA_CD_SEGUI_IL_FILM; 
/
